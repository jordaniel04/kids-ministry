            ...
            <VCardText>
                <VContainer>
                    <VRow>
                        ...
                        <VCol cols="12" sm="6">
                            <VTextField
                                v-model="localItem.ministerialData.totalTeachers"
                        ...
                        <VCol cols="12" sm="6">
                            <VTextField
                                v-model="localItem.ministerialData.totalChildren"
                                label="Total Niños"
                                type="number"
                                readonly
                                :value="totalChildren"
                        ...
                        <VCol cols="12" sm="6">
                            <VTextField
                                v-model="localItem.ministerialData.convertedChildren"
                                ...
                                @focus="clearDefaultValue('convertedChildren')"
                                @input="handleInput($event, 'convertedChildren')"
                                required
                            ></VTextField>
                        </VCol>
                        <VCol cols="12" sm="6">
                            <VTextField
                                v-model="localItem.ministerialData.memberChildren"
                                ...
                                bg-color="success-lighten-5"
                                class="calculation-field"
                                @focus="clearDefaultValue('memberChildren')"
                                required
                            ></VTextField>
                        </VCol>
                        <VCol cols="12" sm="6">
                            <VTextField
                                v-model="localItem.ministerialData.nonRepentantChildren"
                                ...
                                bg-color="success-lighten-5"
                                class="calculation-field"
                                @focus="clearDefaultValue('nonRepentantChildren')"
                                required
                            ></VTextField>
                        </VCol>
                        <VCol cols="12" sm="6">
                            <VTextField
                                v-model="localItem.ministerialData.baptizedChildren"
                                label="Bautizados E.S."
                                type="number"
                                :rules="[rules.nonNegative, rules.graduateRules.baptized]"
                                @focus="clearDefaultValue('baptizedChildren')"
                                @input="handleInput($event, 'baptizedChildren')"
                                required
                            ></VTextField>
                        </VCol>
                        <VCol cols="12" sm="6">
                            <VTextField
                                v-model="localItem.ministerialData.consolidatedGraduates"
                                label="Graduados Consolidados"
                                type="number"
                                :rules="[rules.nonNegative, rules.graduateRules.consolidated]"
                                @focus="clearDefaultValue('consolidatedGraduates')"
                                required
                            ></VTextField>
                        </VCol>
                        <VCol cols="12" sm="6">
                            <VTextField
                                v-model="localItem.ministerialData.sacramentsGraduates"
                                label="Graduados Sacramentos"
                                type="number"
                                :rules="[rules.nonNegative, rules.graduateRules.sacraments]"
                                @focus="clearDefaultValue('sacramentsGraduates')"
                                required
                            ></VTextField>
                        </VCol>
                        <VCol cols="12" sm="6">
                            <VTextField
                                v-model="localItem.ministerialData.rescueClubChildren"
                                label="Club al Rescate"
                                type="number"
                                :rules="[rules.nonNegative, rules.graduateRules.rescue]"
                                @focus="clearDefaultValue('rescueClubChildren')"
                                required
                            ></VTextField>
                        </VCol>
                        <VCol cols="12" sm="6">
                            <VTextField
                                v-model="localItem.ministerialData.discipleshipGraduates"
                                label="Graduados Discipulado"
                                type="number"
                                :rules="[rules.nonNegative, rules.graduateRules.discipleship]"
                                @focus="clearDefaultValue('discipleshipGraduates')"
                                required
                            ></VTextField>
                        </VCol>
                        <VCol cols="12" sm="6">
                            <VTextField
                                v-model="localItem.ministerialData.connection911Children"
                                label="Niños Conexión 911"
                                type="number"
                                :rules="[rules.nonNegative]"
                                @focus="clearDefaultValue('connection911Children')"
                                required
                            ></VTextField>
                        </VCol>
                    </VRow>
...
const editedItem = ref<Church | null>(null);
...
type NumericFields = Exclude<keyof MinisterialData, 'updatedAt' | 'reportPeriodId' | 'totalChildren'>;
...
const getLatestMinisterialData = (church: Church) => {
    if (!church.ministerialData || !Array.isArray(church.ministerialData) || church.ministerialData.length === 0) {
        return {
            totalTeachers: 0,
            totalChildren: 0,
            convertedChildren: 0,
    ...
    return church.ministerialData[0];
};
...
const editItem = (church: Church, index: number) => {
    ...
    // Crear una copia profunda del objeto church
    editedItem.value = { 
        ...church,
        ministerialData: [{
            totalTeachers: ministerialData.totalTeachers || 0,
    ...
    // Asignar los valores al localItem
    localItem.value = {
        name: church.name,
